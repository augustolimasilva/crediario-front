...
export default function VendaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const loadVenda = async () => {
      try {
        ...
        const response = await api.get(`/venda/${resolvedParams.id}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div>
            ...
            <p className="text-lg text-gray-900">{venda.nomeCliente}</p>
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">Data da Venda</p>
            <p className="text-lg text-gray-900">{formatDate(venda.dataVenda)}</p>
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">Vendedor</p>
            <p className="text-lg text-gray-900">{venda.vendedor?.nome || '-'}</p>
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">Usuário</p>
            <p className="text-lg text-gray-900">{venda.usuario?.name || '-'}</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <p className="text-lg font-semibold text-gray-700">
                {formatCurrency(venda.itens.reduce((sum, item) => sum + item.valorTotal, 0))}
              </p>
            </div>
            {venda.desconto && venda.desconto > 0 && (
              <div>
                <p className="text-sm font-medium text-gray-500 mb-1">Desconto</p>
                <p className="text-lg font-semibold text-red-600">- {formatCurrency(venda.desconto)}</p>
              </div>
            )}
            <div>
              <p className="text-sm font-medium text-gray-500 mb-1">Valor Total</p>
              <p className="text-lg font-semibold text-green-600">{formatCurrency(venda.valorTotal)}</p>
            </div>
          </div>
        </div>
        {venda.observacao && (
          <div className="mt-4">
            ...
            <p className="text-gray-900">{venda.observacao}</p>
          </div>
        )}
      </div>
      ...
      {/* Itens da Venda */}
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Itens da Venda</h2>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="text-sm text-gray-500">
            {venda.lancamentosFinanceiros?.length || 0} lançamento(s)
          </span>
        </div>

        {!venda.lancamentosFinanceiros || venda.lancamentosFinanceiros.length === 0 ? (
          ...
          <div className="space-y-4">
            {venda.lancamentosFinanceiros?.map((lancamento) => (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            <div className="p-6">
              ...
              <form onSubmit={(e) => {
                ...
                const formaPagamentoInput = formData.get('formaPagamento');
                const data: any = {
                  valor: valorInput ? Number(valorInput) : undefined,
                  dataPagamento: formData.get('dataPagamento') || null,
                  observacao: formData.get('observacao') || null
                };
                // Só enviar formaPagamento se foi alterada (não vazio)
                if (formaPagamentoInput && formaPagamentoInput.toString().trim()) {
                  data.formaPagamento = formaPagamentoInput.toString();
                }
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      step="0.01"
                      name="valor"
                      defaultValue={lancamentoEditando.valor}
                      required
                  ...
                  <div>
                    ...
                    <select
                      name="formaPagamento"
                      defaultValue={lancamentoEditando.formaPagamento || ''}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                    ...
                    {lancamentoEditando.formaPagamento && (
                      <p className="mt-1 text-xs text-gray-500">
                        Forma atual: {FORMAS_PAGAMENTO.find(f => f.value === lancamentoEditando.formaPagamento)?.label || lancamentoEditando.formaPagamento}
                      </p>
                    )}
                  </div>
                  <div>
